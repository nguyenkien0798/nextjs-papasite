...
import CardWebAppSystem from './components/Card';

function WebAppSystem() {
  const links = [
    {
      title: 'services',
      path: '/services',
  ...
  const listWebAppSystem = [
    {
      ...
      permission2: 'Shop',
      permission3: 'Buyer',
      feature1: 'Many desktop publishing packages ',
      feature2: 'Web page editors now',
      feature3: 'Their default model text and a search',
      feature4: 'Feature n',
      time: '4-6 month',
      technology1: 'ReactJS',
      technology2: 'HTML',
  ...
  return (
    <div>
      <div
        ...
        style={{
          backgroundImage: `url(${BannerWebApp.src})`,
      ...
      <div className="xm:px-[16px] xm:pb-[80px] xm:pt-[24px] sm:px-[32px] sm:pb-[80px] sm:pt-[80px] xl:px-10 2xl:px-[10%]">
        ...
        "
        >
          {listWebAppSystem.map(
            (item) => item && <CardWebAppSystem item={item} key={item.id} />
          )}
        </div>
      </div>
    </div>
